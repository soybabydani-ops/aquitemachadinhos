...
async function aplicar(ext: string | undefined, ativo: boolean) {
  ...
  const table = entity === "driver" ? "drivers" : "stores";
  if (ativo) await supa.from(table).update({ plano: plan, destaque: true }).eq("id", id);
...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (type === "subscription_preapproval") {
      ...
      // se rejeitada/cancelada, desativa; se aprovada, garante ativo
    }
