...
(function () {
  ...
  function renderAdmin(root, stores, offers, met, reviews, drivers, listings) {
    ...
    var stats = '<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">' + statCard('🏢', stores.length, 'Empresas') + statCard('⏳', pend.length, 'Pendentes') + statCard('✅', countBy(stores, 'status', 'ativo'), 'Ativas') + statCard('🔥', countBy(offers, 'status', 'ativa'), 'Ofertas ativas') + statCard('📊', countBy(met, 'tipo', 'pageview'), 'Visitas ao site') + statCard('👁️', countBy(met, 'tipo', 'view'), 'Visualizações') + statCard('💬', countBy(met, 'tipo', 'click_whatsapp'), 'Cliques WhatsApp') + statCard('📍', countBy(met, 'tipo', 'click_mapa'), 'Cliques mapa') + statCard('🚗', (drivers||[]).length, 'Motoristas') + statCard('🟢', countBy(drivers||[], 'disponivel_agora', true), 'Motoristas online') + statCard('📋', (listings||[]).length, 'Anúncios') + '</div>';
