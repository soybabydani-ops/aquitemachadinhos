...
(function () {
  ...
  function reviewList(reviews) {
    ...
    return '<h3 class="font-display font-bold mt-2 mb-3">Comentários</h3><div class="space-y-3">' + reviews.map(function (r) { var d = new Date(r.criado_em).toLocaleDateString('pt-BR'); return '<div class="bg-white rounded-xl ring-silver p-4"><div class="flex items-center gap-2"><div class="w-8 h-8 rounded-full bg-navy-800 text-white grid place-items-center text-xs font-bold">' + esc(initials(r.nome || 'A')) + '</div><div><p class="font-semibold text-sm">' + esc(r.nome || 'Anônimo') + '</p><p class="text-xs text-slate-400">' + d + '</p></div><div class="ml-auto text-sm">' + ratingStars(r.nota) + '</div></div>' + (r.comentario ? '<p class="text-sm text-slate-600 mt-2">' + esc(r.comentario) + '</p>' : '') + '</div>'; }).join('') + '</div>';
  }
  function reviewForm(s) {
    return '<form id="formReview" class="bg-white rounded-2xl ring-silver shadow-soft p-5 mb-4"><h3 class="font-display font-bold mb-3">Avalie esta empresa</h3><div id="starInput" class="flex gap-1 mb-3" data-val="0" role="radiogroup" aria-label="Sua nota de 1 a 5 estrelas">' + [1, 2, 3, 4, 5].map(function (i) { return '<span data-s="' + i + '" class="star cursor-pointer text-3xl text-silver-300 hover:text-amber-400 transition" role="button" tabindex="0" aria-label="' + i + ' estrela' + (i > 1 ? 's' : '') + '">★</span>'; }).join('') + '</div><input name="nome" placeholder="Seu nome (opcional)" class="w-full px-3 py-2 rounded-lg bg-silver-50 ring-silver mb-2"><textarea name="comentario" rows="2" placeholder="Conte sua experiência (opcional)" class="w-full px-3 py-2 rounded-lg bg-silver-50 ring-silver mb-2"></textarea><button class="btn-shine bg-peao-500 hover:bg-peao-600 text-white font-bold px-5 py-2.5 rounded-xl">Enviar avaliação</button><span id="reviewMsg" class="text-sm ml-2"></span><p class="text-xs text-slate-400 mt-2">As avaliações passam por análise antes de serem publicadas.</p></form>';
  }
  ...
  function wireReview(s) {
    ...
    f.addEventListener('submit', function (e) {
      ...
      var o = { store_id: s.id, nota: nota, nome: f.querySelector('[name=nome]').value.trim(), comentario: f.querySelector('[name=comentario]').value.trim(), status: 'pendente' };
  ...
  function painelEditForm(s) {
    ...
    return '<form id="formEdit" class="bg-white rounded-2xl ring-silver shadow-soft p-5 space-y-4 mb-6"><h2 class="font-display font-bold">Editar informações</h2>'
      ...
      + '<label class="flex items-center gap-2 text-sm"><input type="checkbox" name="destaque" class="w-4 h-4 accent-peao-500"' + (s.destaque ? ' checked' : '') + '> Destacar empresa</label>'
  ...
  function wireEdit(s) {
    ...
    f.addEventListener('submit', function (e) {
      ...
      aPatch('stores', s.id, obj).then(function (ok) { var m = $('#editMsg'); if (m) { m.textContent = ok ? 'Salvo!' : 'Erro ao salvar'; m.className = 'text-sm ml-2 ' + (ok ? 'text-emerald-600' : 'text-peao-600'); } });
    });
  }
  ...
  function pageMotorista() {
    ...
    Drivers.get(id).then(function (d) {
      ...
      Drivers.reviews(d.id).then(function (reviews) {
        ...
        wireDriverReview(d); wireAssinar();
      });
    });
  }
  ...
  function wireDriverReview(d) {
    ...
    f.addEventListener('submit', function (e) {
      e.preventDefault(); var msg = $('#reviewMsg'); var nota = parseInt(si.getAttribute('data-val'), 10);
      if (!nota || nota < 1) { msg.className = 'text-sm ml-2 text-peao-600'; msg.textContent = 'Selecione de 1 a 5 estrelas.'; return; }
      var o = { driver_id: d.id, nota: nota, nome: f.querySelector('[name=nome]').value.trim(), comentario: f.querySelector('[name=comentario]').value.trim(), status: 'pendente' };
