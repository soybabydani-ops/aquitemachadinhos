...
(function () {
  ...
  function renderAdmin(root, stores, offers, met, reviews, drivers, listings, cityLeads) {
    ...
    root.querySelectorAll('[data-lead-convert]').forEach(function (btn) { btn.addEventListener('click', function () { var lead = cityLeads.filter(function(x){ return x.id === btn.getAttribute('data-lead-convert'); })[0]; var box = $('#leadConvertRoot'); if (!lead || !box) return; box.innerHTML = leadConvertForm(lead); window.scrollTo(0, 0); var close = $('#closeLeadConvert'); if (close) close.addEventListener('click', function(){ box.innerHTML=''; }); var form = $('#leadConvertForm'); if (form) form.addEventListener('submit', function(e){ e.preventDefault(); var msg=$('#leadConvertMsg'), fd=new FormData(form), citySlug=fd.get('city_slug'), nome=String(fd.get('nome')||'').trim(), categoria=fd.get('categoria'); if(!nome || !categoria){ msg.textContent='Preencha nome e categoria.'; msg.className='sm:col-span-2 text-sm text-red-200'; return; } var submit=form.querySelector('[type=submit]'); submit.disabled=true; submit.textContent='Publicando…'; var obj={ nome:nome, categoria:categoria, whatsapp:String(fd.get('whatsapp')||'').trim(), responsavel:String(fd.get('responsavel')||'').trim(), descricao_curta:String(fd.get('descricao_curta')||'').trim(), cidade:CITY_NAMES[citySlug]||citySlug, city_slug:citySlug, city_lead_id:fd.get('lead_id'), status:'pendente', plano:fd.get('plano')||'gratis', destaque:fd.get('destaque') === 'on', aceite_termos:true, autorizacao_contato:true }; aPost('stores', obj).then(function(arr){ var created=arr&&arr[0]; if(!created){ throw new Error('Não foi possível publicar. Este lead talvez já tenha sido convertido.'); } return aPatch('stores',created.id,{status:'ativo',plano:obj.plano,destaque:obj.destaque}).then(function(ok){if(!ok)throw new Error('Empresa criada, mas não foi possível ativar.'); return aPatch('city_leads',lead.id,{status:'ativo'});}); }).then(function(){ msg.textContent='✓ Empresa publicada em '+(CITY_NAMES[citySlug]||citySlug)+'!'; msg.className='sm:col-span-2 text-sm text-emerald-200'; setTimeout(pageAdmin,800); }).catch(function(err){ msg.textContent=err.message||'Erro ao publicar.'; msg.className='sm:col-span-2 text-sm text-red-200'; submit.disabled=false; submit.textContent='Revisar e publicar empresa →'; }); }); }); });
