...
const {PARTNERS}=require('../api/affiliate-redirect');
...
for(const file of files.filter(f=>f.endsWith('.html'))){
  ...
  for(const match of html.matchAll(/affiliate-redirect\?partner=([^&"]+)/g)) assert(PARTNERS[decodeURIComponent(match[1])],`unknown partner in ${file}`);
}
for(const [key,p] of Object.entries(PARTNERS)){
  const host=new URL(p.url).hostname;
  assert(['s.shopee.com.br','onelink.shein.com','meli.la','link.amazon','udemy.sjv.io'].includes(host),`unapproved host for ${key}`);
}
...
console.log(JSON.stringify({ok:true,htmlFiles:449,hydrationRows:448,canonicalUrls:449,affiliatePartners:Object.keys(PARTNERS),redirects:'legacy urgency routes canonicalized'},null,2));
